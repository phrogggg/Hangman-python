...
def game():
    ...
    if lives==0:
        print(image[0])
        cont=input(f"You died.The word was {word}. Do you want to play again (Y) or quit (Q)?").upper()
        if cont=="Y":
            game()
        elif cont == "Q":
    ...
    #asks user if the want to quit or continue if they win
        ...
        c=input(f"Great job! You guessed the word: {word}. Do you want to play again (Y) or quit (Q): ").upper()
        if c == "Y":
            game()
        elif c == "Q":
            print("Exiting in 5 seconds")
